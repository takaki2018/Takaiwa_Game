...
#define HEIGHT_Y		(-100.0f)					// タイトルロゴの初期中心Y座標

#define HEIGHT_DEST		(250.0f)					// 目的の高さ
...
void UpdateTitlelogo(void)
{
	...
	{// エンターキーを押したとき
		...
		{// フェードインの時
			...
			g_posTitleLogo.y = HEIGHT_DEST;
			g_bFall = true;
		}
		else if (g_posTitleLogo.y <= HEIGHT_DEST)
		{// タイトルロゴが落ちてきているとき
			g_posTitleLogo.y = HEIGHT_DEST;
			g_bFall = true;
		}
	...
	// 更新処理
	if (nFade == FADE_NONE && g_posTitleLogo.y != HEIGHT_DEST)
	{// タイトルロゴの移動量の更新
