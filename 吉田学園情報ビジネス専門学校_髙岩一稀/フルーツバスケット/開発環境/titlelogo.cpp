...
void UpdateTitlelogo(void)
{
	...
	{// ロゴを回転させる
		...
		int nNum = 0;
		nNum = g_TLCnt % TITLELOGO_ROT_COUNTER;

		if ((0 <= (g_TLCnt % TITLELOGO_ROT_COUNTER)) && (50 > (g_TLCnt % TITLELOGO_ROT_COUNTER)))
		{// 左の傾き
			if ((0 <= (g_TLCnt % TITLELOGO_ROT_COUNTER)) && (25 > (g_TLCnt % TITLELOGO_ROT_COUNTER)))
			{
				g_TitleLogo.rot.z += TITLELOGO_ROT_Z;
			}
			else if ((25 <= (g_TLCnt % TITLELOGO_ROT_COUNTER)) && (50 > (g_TLCnt % TITLELOGO_ROT_COUNTER)))
			{
				g_TitleLogo.rot.z -= TITLELOGO_ROT_Z;
			}
		}
		else if ((50 <= (g_TLCnt % TITLELOGO_ROT_COUNTER)) && (TITLELOGO_ROT_COUNTER > (g_TLCnt % TITLELOGO_ROT_COUNTER)))
		{// 右の傾き
			if ((50 <= (g_TLCnt % TITLELOGO_ROT_COUNTER)) && (75 > (g_TLCnt % TITLELOGO_ROT_COUNTER)))
			{
				g_TitleLogo.rot.z -= TITLELOGO_ROT_Z;
			}
			else if ((75 <= (g_TLCnt % TITLELOGO_ROT_COUNTER)) && (100 > (g_TLCnt % TITLELOGO_ROT_COUNTER)))
			{
				g_TitleLogo.rot.z += TITLELOGO_ROT_Z;
			}
		}
