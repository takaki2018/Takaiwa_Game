...
#define PLAYER_SIZE_CORRECTION		(0.7f)														// プレイヤーのサイズ補正用マクロ
...
void SingleMovePlayer(void)
{
	...
	{// 穴のある座標の高さにあるとき
		if (g_aPlayer[PLAYER_1].pos.x - g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION <= 50.0f)
		{// 左側の穴にいるとき左側の木の上から出る
			g_aPlayer[PLAYER_1].pos.x = g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION + 50.0f;
			g_aPlayer[PLAYER_1].pos.y = 270.0f;
		}
		else if (g_aPlayer[PLAYER_1].pos.x - g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION >= SCREEN_WIDTH - 50.0f)
		{// 右側の穴にいるとき右側の木の上から出る
			g_aPlayer[PLAYER_1].pos.x = SCREEN_WIDTH - 50.0f - g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION;
			g_aPlayer[PLAYER_1].pos.y = 270.0f;
		}
	}
	else if (g_aPlayer[PLAYER_1].pos.x - g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION <= WOOD_WIDTH)
	{// 一番左に到達したとき処理
		g_aPlayer[PLAYER_1].pos.x = g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION + WOOD_WIDTH;
	}
	else if (g_aPlayer[PLAYER_1].pos.x + g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION >= SCREEN_WIDTH - WOOD_WIDTH)
	{// 一番右に到達したとき処理
		g_aPlayer[PLAYER_1].pos.x = SCREEN_WIDTH - WOOD_WIDTH - g_aPlayer[PLAYER_1].fWidth * PLAYER_SIZE_CORRECTION;
	}
...
void MultiMovePlayer(int nCntPlayer)
{
	...
	{// 穴のある座標の高さにあるとき
		if (g_aPlayer[nCntPlayer].pos.x - g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION <= 50.0f)
		{// 左側の穴にいるとき左側の木の上から出る
			g_aPlayer[nCntPlayer].pos.x = g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION + 50.0f;
			g_aPlayer[nCntPlayer].pos.y = 270.0f;
		}
		else if (g_aPlayer[nCntPlayer].pos.x - g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION >= SCREEN_WIDTH - 50.0f)
		{// 右側の穴にいるとき右側の木の上から出る
			g_aPlayer[nCntPlayer].pos.x = SCREEN_WIDTH - 50.0f - g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION;
			g_aPlayer[nCntPlayer].pos.y = 270.0f;
		}
	}
	else if (g_aPlayer[nCntPlayer].pos.x - g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION <= WOOD_WIDTH)
	{// 一番左に到達したとき処理
		g_aPlayer[nCntPlayer].pos.x = g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION + WOOD_WIDTH;
	}
	else if (g_aPlayer[nCntPlayer].pos.x + g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION >= SCREEN_WIDTH - WOOD_WIDTH)
	{// 一番右に到達したとき処理
		g_aPlayer[nCntPlayer].pos.x = SCREEN_WIDTH - WOOD_WIDTH - g_aPlayer[nCntPlayer].fWidth * PLAYER_SIZE_CORRECTION;
	}
