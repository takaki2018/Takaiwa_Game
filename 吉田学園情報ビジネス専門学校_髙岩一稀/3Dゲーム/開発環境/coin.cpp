...
HRESULT InitCoin(void)
{
	...
	for (int nCntCoin = 0; nCntCoin < g_CoinInfo.nCntCoin; nCntCoin++)
	{
		...
		// 影の配置
		g_CoinInfo.coinSet[nCntCoin].nIdxShadow = SetShadow(D3DXVECTOR3(g_CoinInfo.coinSet[nCntCoin].pos.x, 0.0f, g_CoinInfo.coinSet[nCntCoin].pos.z), SHADOW_SIZE, SHADOW_SIZE);
	}
