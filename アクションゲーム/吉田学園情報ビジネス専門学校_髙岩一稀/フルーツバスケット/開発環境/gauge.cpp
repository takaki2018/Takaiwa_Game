...
HRESULT InitGauge(void)
{
	...
	for (int nCntBar = 0; nCntBar < MAX_PATTERN; nCntBar++)
	{
		...
		g_aGauge[nCntBar].nRank = RANKING_FIRST;
...
void UpdateGauge(void)
{
	...
	for (int nCnt = 0; nCnt < g_nDisplayGauge; nCnt++, pPlayer++)
	{
		// 変数宣言
		...
		{// 自分のスコアに到達したら処理
			...
			{
				SetResultLogo(pPlayer->pos, g_aGauge[nCnt].fMoveHeight, (RANK)g_aGauge[nCnt].nRank);	// 何位か設置
			}
