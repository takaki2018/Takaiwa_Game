...
#define MAX_PATTERN				(11)		// 背景の種類
#define MAX_TEXTURE				(16)		// 貼るテクスチャの最大数
...
int g_aScore[MAX_RANKING_RANK] = { 50,40,30,20,10 };		// ランキングの初期スコア
...
HRESULT InitRanking(void)
{
	...
	{
		g_ScoreCol[nCnt] = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	}

	// テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/ranking_logo.png", &g_pTextureRanking[0]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/ranking_1st.png", &g_pTextureRanking[1]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/ranking_2nd.png", &g_pTextureRanking[2]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/ranking_3rd.png", &g_pTextureRanking[3]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/ranking_4th.png", &g_pTextureRanking[4]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/ranking_5th.png", &g_pTextureRanking[5]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/number000.png", &g_pTextureRanking[6]);
...
void UpdateRanking(void)
{
	...
	// スコアのテクスチャ更新
	for (int nCntScore = 6; nCntScore < MAX_PATTERN; nCntScore++)
	{// 頂点座標の更新
...
void DrawRanking(void)
{
	...
	for (int nCntRanking = 0; nCntRanking < MAX_TEXTURE; nCntRanking++)
	{
		...
		else if (nCntRanking >= 6)
		{
			pDevice->SetTexture(0, g_pTextureRanking[6]);
		}
...
void SetVertexRanking(int nCnt)
{
	...
	else if (nCnt >= 6 && nCnt < MAX_TEXTURE)
	{// 点滅
		if (g_bFlash == true)
		{
...
void RankingRank(int nCnt)
{
	...
	// 順位を配置する間隔
	int nCntWidth = (nCnt - 1) % MAX_RANKING_RANK;
...
void RankingScore(int nCnt)
{
	...
	int nNumScore = g_nCntScore % (MAX_RANKING_RANK - 1);		// 何番目のスコアか
