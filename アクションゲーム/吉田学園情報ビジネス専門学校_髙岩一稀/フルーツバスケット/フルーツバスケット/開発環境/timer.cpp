...
// マクロ定義
//-------------------------------------------------------------------
#define TIMER_X_SINGLE			(795.0f)		// シングルモードのタイマーの中心座標(X)
#define TIMER_Y_SINGLE			(65.0f)			// シングルモードのタイマーの中心座標(Y)
#define TIMER_X_MULTI			(595.0f)		// マルチモードのタイマーの中心座標(X)
#define TIMER_Y_MULTI			(60.0f)			// マルチモードのタイマーの中心座標(Y)
...
HRESULT InitTimer(void)
{
	...
	case PLAYMODE_SINGLE:
		g_posTimer = D3DXVECTOR3(TIMER_X_SINGLE, TIMER_Y_SINGLE, 0.0f);
		break;
	// マルチモードのとき処理
	case PLAYMODE_MULTI:
		g_posTimer = D3DXVECTOR3(TIMER_X_MULTI, TIMER_Y_MULTI, 0.0f);
		break;
	}
	g_nTimer = TIME;								// 時間			
	g_nTimerCnt = TIMECOUNT_INTERBAL;				// タイマーカウント
	g_nCntFlashTimer = 0;
